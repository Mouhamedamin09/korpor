...
import { fetchUserSettings, UserSettings } from "@main/services/api";
...
const ConfirmAutoInvest: React.FC<Props> = ({
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        setLoading(true);
        // First get account data to get the email
        const account = await fetchAccountData();
        setAccountData(account);

        // Then fetch wallet and user settings
  ...
  // Check if account is verified (using a safe default since the property structure is unclear)
  const isAccountVerified = false; // Set to false for now - you can add verification status to AccountData type later
