 ...
 */
export const convertCurrency = (amount: number, fromCurrency: 'TND' | 'USD' | 'EUR', toCurrency: 'TND' | 'USD' | 'EUR'): number => {
  if (fromCurrency === toCurrency) return amount;
  
  // Convert from source currency to TND first
  const amountInTND = fromCurrency === 'TND' ? amount : amount / EXCHANGE_RATES[fromCurrency];
  
  // Convert from TND to target currency
  const convertedAmount = toCurrency === 'TND' ? amountInTND : amountInTND * EXCHANGE_RATES[toCurrency];
 ...
 */
export const formatBalance = (amount: number, fromCurrency: 'TND' | 'USD' | 'EUR', displayCurrency: 'USD' | 'EUR' | 'TND'): string => {
 ...
 */
export const depositFunds = async (depositData: DepositRequest): Promise<any> => {
  try {
    const headers = await getAuthHeaders();
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to deposit funds');
  }
};
 ...
 */
export const withdrawFunds = async (withdrawData: WithdrawRequest): Promise<any> => {
  try {
    const headers = await getAuthHeaders();
      ...
      const errorData = await response.json();
        ...
        if (response.status === 400) {
          if (errorMessage.toLowerCase().includes('insufficient')) {
            ...
            errorMessage = 'Invalid withdrawal amount. Please check the minimum and maximum limits.';
 ...
 */
export const addRewards = async (rewardsData: AddRewardsRequest): Promise<any> => {
  try {
    const headers = await getAuthHeaders();
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to add rewards');
  }
}; 