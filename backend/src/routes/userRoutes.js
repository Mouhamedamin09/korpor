...
router.post('/request-:field-change', async (req, res) => {
  ...
  try {
    ...
    const newValue = req.body[`new${field.charAt(0).toUpperCase() + field.slice(1)}`];
    ...
    const lastChangeField = field === 'phone' ? 'lastPhoneChange' : 'lastEmailChange';
    ...
    if (changeCount >= 5) {
      console.log(`❌ ${field} change not allowed - reached monthly limit of 5 changes`);
    ...
    try {
      if (field === 'phone') {
        ...
        await sendVerificationEmail(user.email, verificationCode, user.name || 'User', field);
      }
      console.log(`✅ ${field === 'phone' ? 'SMS' : 'Email'} sent successfully`);
    } catch (error) {
      console.error(`❌ Failed to send ${field === 'phone' ? 'SMS' : 'email'}:`, error);
      return res.status(500).json({ error: `Failed to send verification ${field === 'phone' ? 'SMS' : 'email'}` });
    }
...
router.post('/verify-:field', async (req, res) => {
  ...
  try {
    ...
    const lastChangeField = field === 'phone' ? 'lastPhoneChange' : 'lastEmailChange';
    const verificationCodeField = field === 'phone' ? 'phoneVerificationCode' : 'emailVerificationCode';
